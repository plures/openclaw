...
'@
$nl = Get-Newline $text
$oldScanHelper = Convert-Newlines $oldScanHelper $nl
if (-not $text.Contains($oldScanHelper)) {
    throw "remove transcript message scan helper: anchor not found."
}
$text = $text.Replace($oldScanHelper, "")
$changes[$path] = $text
