...
$patchPath = Join-Path $PSScriptRoot "openclaw-usage-cache-worker.patch"
...
try {
    ...
    if ($CheckOnly) {
        Write-Host "Usage worker patch preflight passed for $repo"
        return
    }
    ...
    $workerOutput = Join-Path $dist "gateway/usage.worker.js"
    ...
    if (-not $SkipTests) {
        Invoke-Native pnpm exec vitest run src/gateway/usage-worker-client.test.ts src/gateway/server-methods/usage.status-cache.test.ts src/gateway/server-methods/usage.test.ts
    }

    Write-Host "Usage patch applied and verified. Worker output: $workerOutput"
